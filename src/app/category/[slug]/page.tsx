...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const [categoryRes, resourceCountRes, subcategoriesRes] = await Promise.all([
    ...
    supabase.from("resources").select("id", { count: "exact" }).eq("status", "approved").eq("category", slug),
    supabase.from("subcategories").select("name").eq("category_id", "").limit(5)
  ]);
  ...
  const categoryKeywords = {
    Frontend: ["react", "vue", "angular", "svelte", "css", "html", "javascript", "typescript", "ui components", "frontend frameworks"],
    Backend: ["nodejs", "python", "java", "golang", "rust", "apis", "databases", "servers", "backend frameworks", "microservices"],
    DevOps: ["docker", "kubernetes", "ci/cd", "aws", "azure", "gcp", "monitoring", "deployment", "infrastructure", "automation"],
    Design: ["figma", "sketch", "adobe", "ui design", "ux design", "prototyping", "design systems", "icons", "fonts", "colors"],
    Tools: ["vscode", "git", "npm", "webpack", "vite", "eslint", "prettier", "testing", "debugging", "productivity"],
    Learning: ["tutorials", "courses", "documentation", "books", "videos", "blogs", "guides", "examples", "best practices"]
  };
  ...
  const keywords = [
    ...
    "developer resources", "programming tools", "thesupadevs"
  ];

  if (subcategory) {
    keywords.unshift(subcategory.toLowerCase(), `${subcategory.toLowerCase()} tools`);
  }
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      url: subcategory 
        ? `https://thesupadevs.vercel.app/category/${slug}?subcategory=${encodeURIComponent(subcategory)}`
        : `https://thesupadevs.vercel.app/category/${slug}`,
      siteName: "TheSupaDevs",
      images: [{
        url: `/og-${categoryData.name.toLowerCase()}.png`,
        width: 1200,
        height: 630,
    ...
    twitter: {
      ...
      images: [`/og-${categoryData.name.toLowerCase()}.png`]
    },
    alternates: {
